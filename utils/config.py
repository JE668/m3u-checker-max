...
# P1-12: 线程安全 Session / 共享线程池 / 请求重试 — 已拆分至 utils/http.py。
# 此处仅做兼容再导出（既有调用方与测试无需改动）。
from utils.http import fetch_url, get_pool, get_session, retry_request  # noqa: F401
...
# 1.5 网络工具：重试装饰器 (P1-6)
# ===============================
# retry_request / fetch_url 已移至 utils/http.py（见文件顶部兼容再导出）
